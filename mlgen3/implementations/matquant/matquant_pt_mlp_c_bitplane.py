...
class MatQuantPT_C_Bitplane(Implementation):
    """
    Pure C implementation of MatQuant for MLP models with bitplane packing.
    ...
    def _to_signed_8bit_binary(self, val):
        """Convert a signed 8-bit integer to binary string."""
        if val < 0:
            val = (1 << 8) + val  # Two's complement
        return f"0b{val:08b}"
    
    def _format_array_1d_binary(self, values, name, dtype_str="int8_t", items_per_line=8):
        """Format a 1D array as C code with binary representation."""
        values = np.array(values).flatten()
        lines = []
        for i in range(0, len(values), items_per_line):
            chunk = values[i:min(i + items_per_line, len(values))]
            chunk_str = ", ".join(self._to_signed_8bit_binary(int(v)) for v in chunk)
            lines.append(f"    {chunk_str}")
        values_str = ",\n".join(lines)
        return f"/* Binary representation */\nstatic const {dtype_str} {name}_binary[{len(values)}] = {{\n{values_str}\n}};\n"
    
    def _format_array_2d_binary(self, values, name, rows, cols, dtype_str="int8_t", items_per_line=8):
        """Format a 2D array as C code with binary representation."""
        values = np.array(values).reshape(rows, cols)
        lines = []
        for i in range(rows):
            row = values[i]
            ...
            lines.append(f"    {{\n        {row_str}\n    }}")
        values_str = ",\n".join(lines)
        return f"/* Binary representation */\nstatic const {dtype_str} {name}_binary[{rows}][{cols}] = {{\n{values_str}\n}};\n"
...
/* 
 * Weights are packed in bitplane-interleaved format (16-value blocks).
 * This allows extracting lower bit-width versions by reading fewer bytes:
 * - 2-bit: read bytes 0-3 per 16-value block (first bitplane)
 * - 4-bit: read bytes 0-7 per 16-value block (first two bitplanes)
 * - 8-bit: read all 16 bytes per block (all four bitplanes)
 * 
 * Packing strategy per 16-value block:
 * - Bits [7:6] from values 0-15 packed into bytes 0-3
 * - Bits [5:4] from values 0-15 packed into bytes 4-7
 * - Bits [3:2] from values 0-15 packed into bytes 8-11
 * - Bits [1:0] from values 0-15 packed into bytes 12-15
 */
 ...
 * Both decimal and binary representations are provided.
 */
...
"""
        ...
        for lid, layer in enumerate(self.model.layers):
            if isinstance(layer, Linear):
                ...
                # Add PACKED weights to main header (decimal)
                header_code += f"/* Linear layer {lid} weights - BITPLANE PACKED (decimal) */\n"
                header_code += f"/* Original shape: [{layer.output_shape}][{layer.input_shape}] */\n"
                header_code += f"/* Packed shape: [{layer.output_shape}][{packed_weights.shape[1]}] (padded to 16-multiple) */\n"
                header_code += self._format_array_2d(
                    packed_weights, 
                    f"layer_{lid}_lin_weight_q8_packed",
                    layer.output_shape, packed_weights.shape[1], int_type
                )
                
                # Add PACKED weights to main header (binary)
                header_code += self._format_array_2d_binary(
                    packed_weights,
                    f"layer_{lid}_lin_weight_q8_packed",
                    layer.output_shape, packed_weights.shape[1], int_type
                )
                ...
                # Add UNPACKED weights to debug header (decimal)
                debug_header_code += f"/* Linear layer {lid} weights - UNPACKED (original quantized, decimal) */\n"
                ...
                # Add UNPACKED weights to debug header (binary)
                debug_header_code += self._format_array_2d_binary(
                    quantized_weights,
                    f"layer_{lid}_lin_weight_q8_unpacked",
                    layer.output_shape, layer.input_shape, int_type
                )
                ...
                if self.use_bias and layer.bias is not None:
                    ...
                    # Add PACKED bias to main header (decimal)
                    header_code += f"/* Linear layer {lid} bias - BITPLANE PACKED (decimal) */\n"
                    header_code += f"/* Original length: {layer.output_shape}, Packed length: {len(packed_bias)} */\n"
                    header_code += self._format_array_1d(
                        packed_bias,
                        f"layer_{lid}_lin_bias_q8_packed", int_type
                    )
                    
                    # Add PACKED bias to main header (binary)
                    header_code += self._format_array_1d_binary(
                        packed_bias,
                        f"layer_{lid}_lin_bias_q8_packed", int_type
                    )
                    ...
                    # Add UNPACKED bias to debug header (decimal)
                    debug_header_code += f"/* Linear layer {lid} bias - UNPACKED (original quantized, decimal) */\n"
                    ...
                    # Add UNPACKED bias to debug header (binary)
                    debug_header_code += self._format_array_1d_binary(
                        quantized_bias,
                        f"layer_{lid}_lin_bias_q8_unpacked", int_type
                    )
                    debug_header_code += "\n"
                ...
                # Add PACKED scale (decimal and binary)
                header_code += f"/* BatchNorm layer {lid} scale - BITPLANE PACKED (decimal) */\n"
                header_code += self._format_array_1d(
                    packed_scale,
                    f"layer_{lid}_bn_scale_q8_packed", int_type
                )
                header_code += self._format_array_1d_binary(
                    packed_scale,
                    f"layer_{lid}_bn_scale_q8_packed", int_type
                ...
                # Add UNPACKED scale (decimal and binary)
                debug_header_code += f"/* BatchNorm layer {lid} scale - UNPACKED (decimal) */\n"
                debug_header_code += f"/* Shape: [{layer.output_shape}] */\n"
                debug_header_code += self._format_array_1d(
                    quantized_scale,
                ...
                # Add PACKED bias (decimal and binary)
                header_code += f"/* BatchNorm layer {lid} bias - BITPLANE PACKED (decimal) */\n"
                header_code += self._format_array_1d(
                    packed_bias,
                    f"layer_{lid}_bn_bias_q8_packed", int_type
                )
                header_code += self._format_array_1d_binary(
                    packed_bias,
                    f"layer_{lid}_bn_bias_q8_packed", int_type
                ...
                # Add UNPACKED bias (decimal and binary)
                debug_header_code += f"/* BatchNorm layer {lid} bias - UNPACKED (decimal) */\n"
                debug_header_code += f"/* Shape: [{layer.output_shape}] */\n"
                debug_header_code += self._format_array_1d(
                    quantized_bias,
