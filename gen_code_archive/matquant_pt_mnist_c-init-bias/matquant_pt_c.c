...
static int8_t layer_3_lin_bias_q8[512];
static float layer_3_lin_bias_scale;
static float layer_3_lin_bias_zero_point;
...
static int8_t layer_6_lin_bias_q8[10];
static float layer_6_lin_bias_scale;
static float layer_6_lin_bias_zero_point;
...
int load_model_parameters(void) {
    ...
    if (!load_binary_file("mq_pt_model_binary/layer_3_lin_bias.bin",
                         layer_3_lin_bias_q8,
                         sizeof(layer_3_lin_bias_q8))) return 0;
    if (!load_qparams("mq_pt_model_binary/layer_3_lin_bias_qparams.bin",
                     &layer_3_lin_bias_scale,
                     &layer_3_lin_bias_zero_point)) return 0;
    ...
    if (!load_binary_file("mq_pt_model_binary/layer_6_lin_bias.bin",
                         layer_6_lin_bias_q8,
                         sizeof(layer_6_lin_bias_q8))) return 0;
    if (!load_qparams("mq_pt_model_binary/layer_6_lin_bias_qparams.bin",
                     &layer_6_lin_bias_scale,
                     &layer_6_lin_bias_zero_point)) return 0;
...
void predict(const float* input, float* output, int input_size, int output_size) {
    ...
    /* Linear layer 3 with bias */
    for (int i = 0; i < 512; i++) {
        float acc = dequantize(layer_3_lin_bias_q8[i], 
                             layer_3_lin_bias_scale, 
                             layer_3_lin_bias_zero_point);
        for (int j = 0; j < 512; j++) {
    ...
    /* Linear layer 6 with bias */
    for (int i = 0; i < 10; i++) {
        float acc = dequantize(layer_6_lin_bias_q8[i], 
                             layer_6_lin_bias_scale, 
                             layer_6_lin_bias_zero_point);
        for (int j = 0; j < 512; j++) {
