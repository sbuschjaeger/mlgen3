...
class TestNeuralNetwork(unittest.TestCase):
    ...
    def test_bnn_linuxstandalone(self):
        ...
        layers = [
            Linear(np.random.choice([-1,1],size=(84,self.X.shape[1])), np.random.choice([-1,1],size=84)),
            BatchNorm(np.random.uniform(size=84),np.random.uniform(size=84),np.random.uniform(size=84),np.random.uniform(size=84), 1e-5),
            Step(84),
            Linear(np.random.choice([-1,1],size=(32,84)), np.random.choice([-1,1],size=32)),
